    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-lg p-4">
      ...
      <div className="flex flex-col w-full max-w-160.5 bg-white/20 border-2 border-border-muted rounded-[26.75px] shadow-[0_42.8px_133.7px_rgba(0,0,0,0.5)] overflow-hidden">
        ...
        <div className="flex flex-col items-start p-[32.1px_37.45px_0px_37.45px] w-full">
          ...
          <p className="pt-2 pb-5.25 text-[17.39px] font-normal text-text-muted leading-6.5 w-full">
          ...
          <div className="w-full relative">
            <textarea 
              value={appData.introduction}
              onChange={(e) => setAppData({ ...appData, introduction: e.target.value })}
              maxLength={1000}
              placeholder="Introduce yourself and explain why you're a great fit..."
              className="w-full h-46.75 bg-black/20 border-2 border-black/5 rounded-2xl p-[18.73px_21.4px] text-[17.39px] text-white placeholder:text-text-muted resize-none focus:outline-none focus:border-primary-green transition-colors"
            />
          </div>

          {/* Text Area Footer */}
          <div className="flex justify-between items-start pt-2 w-full mb-9.5">
            <span className="text-[14.71px] font-normal text-white/[0.28] leading-5.5">
              Auto-saved as draft
            </span>
            <span className="text-[14.71px] font-normal text-white/[0.28] leading-5.5">
              {appData.introduction.length} / 1000
            </span>
          </div>
        </div>

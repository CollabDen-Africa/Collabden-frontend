...
import { useUserAuditHistory } from "@/hooks/admin/useUserAuditHistory";
import { useUserNotes, useAddUserNote } from "@/hooks/admin/useUserNotes";
import { Pagination } from "@/components/ui/Pagination";
import { useAuth } from "@/context/AuthContext";

export const UserNotesAudit: React.FC<{ userId: string }> = ({ userId }) => {
  ...
  const { user } = useAuth();
  const { data: auditData, isLoading: isAuditLoading } = useUserAuditHistory({ id: userId, page: auditPage, limit: 10 });
  const { data: notesData, isLoading: isNotesLoading } = useUserNotes({ id: userId, page: notesPage, limit: 5 });
  const { mutateAsync: addNote } = useAddUserNote(userId);

  const handleAddNote = async () => {
    if (!newNoteContent.trim() || !user?.id) return;
    setIsAddingNote(true);
    try {
      await addNote({ content: newNoteContent, adminId: user.id });
      setNewNoteContent("");

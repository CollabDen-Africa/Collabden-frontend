...
interface ProfileMiddleColumnProps {
  bio?: string | null;
          ...
          <div className="grid grid-cols-2 gap-4 animate-in fade-in duration-300">
            {portfolio.map((item) => (
              <div key={item.id} className="h-45 border border-white/10 rounded-[14px] relative overflow-hidden group cursor-pointer">
                ...
                <div className="absolute inset-0 bg-zinc-800 bg-cover bg-center transition-transform duration-500 group-hover:scale-105" style={item.image ? { backgroundImage: `url(${item.image})` } : undefined} />
                <div className="absolute inset-0 bg-linear-to-t from-black/80 via-black/20 to-transparent z-10" />
                
                <div className="absolute top-2 right-2 px-2.5 py-0.5 rounded-[30px] shadow-sm z-20 bg-primary-green">
                ...
                <div className="absolute bottom-4 left-4 flex flex-col z-20">
                  ...
                  <span className="font-raleway font-normal text-[12px] text-white/60 leading-4.5 mt-1">
                    {item.role || "Portfolio project"}
                  </span>
                </div>
              </div>
            ))}
            {!portfolio.length && <p className="col-span-2 text-center py-8 text-white/45 text-sm">No portfolio projects yet.</p>}
          </div>
        )}
          ...
          <div className="flex flex-col gap-3 animate-in fade-in duration-300">
            {portfolio.map((item) => (
              <div key={item.id} className="flex flex-row items-center gap-4 p-2.5 border border-white/10 rounded-[14px] bg-black/20 hover:bg-white/5 transition-all cursor-pointer group">
                ...
                {/* Status Pill */}
                <div className="px-3 py-1 rounded-[30px] shadow-sm shrink-0 mr-1.5 bg-primary-green/10 text-primary-green border border-primary-green/20">
            ...
            {!portfolio.length && <p className="text-center py-8 text-white/45 text-sm">No portfolio projects yet.</p>}
          </div>
        )}

...
function AdminVerifyContent() {
  ...
  // Timer logic for 2 minutes (120 seconds) to match backend
  const [timeLeft, setTimeLeft] = useState(120);

  useEffect(() => {
    // Only redirect after a delay if still missing params
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      setTimeLeft(120); // Restart 2 min timer
      ...
      <div className="flex items-center justify-between w-full gap-2 md:gap-3 mb-8">
        {otp.map((digit, index) => (
          <input
            ...
            className={`w-12 h-14 md:w-16 md:h-20 rounded-[14px] bg-transparent border ${digit ? "border-[#72c043] text-white" : "border-white/10 text-white/50"} text-center text-xl md:text-2xl font-medium focus:border-[#72c043] focus:bg-white/[0.02] outline-none transition-all`}
          />
        ))}
      </div>
